...
export const getProduct = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: "Product is listed.", data: product });
  } catch (error) {
    res.status(404).json({ success: false, message: "Product is not found."});
  }
};
...
export const postProduct = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, message: "New product is created.", data: newProduct });
  } catch (error) {
